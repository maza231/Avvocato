...
export default async function ServiceDetail({params}){
  ...
  return <main>
    ...
    <nav className="border-b border-black/10 bg-[#173b63] text-white" aria-label="Navigazione servizi"><div className="container flex gap-1 overflow-x-auto py-3">{services.map((item,i)=><Link key={item.slug} href={`/servizi/${item.slug}`} aria-current={item.slug===slug?'page':undefined} className={`shrink-0 border px-4 py-3 text-[10px] font-bold uppercase tracking-[.12em] transition-colors ${item.slug===slug?'border-white bg-white text-[#173b63]':'border-white/15 text-white/70 hover:border-white/50 hover:text-white'}`}>0{i+1} · {item.title}</Link>)}</div></nav>
